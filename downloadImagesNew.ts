...
const imagesToDownload = [
  ...
  // Extra Blog Images from constants.ts (Replacing picsum placeholders with related content from our mapping)
...
async function downloadAll() {
  for (const img of imagesToDownload) {
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (buffer.byteLength > 100) { // avoid empty corrupt files
            fs.writeFileSync(dest, Buffer.from(buffer));
            console.log('Saved', dest);
        } else {
            console.error('File too small, possibly corrupt for', img.url)
        }
      } else {
        console.error('HTTP', res.status, img.url);
      }
    } catch (e) {
  ...
  function walk(directory) {
    ...
    for (const file of files) {
      ...
      if (fs.statSync(p).isDirectory() && file !== 'node_modules') walk(p);
